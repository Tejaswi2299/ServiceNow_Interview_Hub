...
function updateSidebarStats() {
  ...
  document.getElementById('stat-roles').textContent = counts.roles;
  document.getElementById('stat-modules').textContent = counts.modules;
  document.getElementById('stat-topics').textContent = counts.topics;
  document.getElementById('stat-items').textContent = counts.totalSeededStudyItems;
}
...
function renderRoute() {
  ...
  if (segments[0] === 'roles' && segments[1]) {
    ...
    const related = content.filter((item) => (item.roleIds || []).includes(role.id));
  ...
  if (segments[0] === 'modules' && segments[1]) {
    ...
    const related = content.filter((item) => (item.moduleIds || []).includes(module.id));
