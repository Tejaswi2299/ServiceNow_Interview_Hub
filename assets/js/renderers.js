...
function entityCard(entity, type, lookups = {}) {
  ...
  return `
    <a class="card item-card entity-card entity-card-link entity-card-${escapeHtml(typeLabel.toLowerCase())}" href="${route}" data-entity-type="${escapeHtml(typeLabel.toLowerCase())}" data-entity-id="${escapeHtml(entity.id)}" aria-label="Open ${escapeHtml(entity.name)}">
...
function groupedTopicAccordions(items, lookups, titleFallback = 'General') {
  ...
  return `
    <a class="study-row" href="${linkForItem(item)}">
      ...
      <div class="study-row-meta">
        <span class="badge ${typeTone === 'green' ? 'green' : typeTone === 'orange' ? 'orange' : ''}">${escapeHtml(labelForContentType(item.contentType))}</span>
...
export function renderRoleDetail(state, role, related) {
  ...
  const topicGroups = groupedTopicAccordions(related, state.lookups, 'Role topics');
  return `
    ...
    <section class="card entity-detail-hero compact-detail-hero">
      ...
      ${modules.length ? `<div class="meta-inline role-module-pills">${modules.slice(0, 8).map((module) => `<a class="badge subtle" href="#/modules/${module.slug}">${escapeHtml(module.name)}</a>`).join('')}</div>` : ''}
    </section>
    ${renderAccordionSet(topicGroups, 'Study by topic', 'Open a topic to jump directly into the questions mapped to this role.')}
  `;
}
...
export function renderModuleDetail(state, module, related) {
  const topicGroups = groupedTopicAccordions(related, state.lookups, 'Module topics');
  return `
    ...
    ${renderAccordionSet(topicGroups, 'Study by topic', 'Open a topic group to see the questions mapped to this module.')}
  `;
}
  ...
  return `
    ...
    <section class="card entity-detail-hero compact-detail-hero">
      <div class="entity-card-top">
        ...
        <div class="entity-copy">
          <div class="badges"><span class="badge">Topic</span><span class="badge subtle">${escapeHtml(topic.category)}</span></div>
          <h2>${escapeHtml(topic.name)}</h2>
          <p>Open each section below to study the exact questions mapped to this topic.</p>
        </div>
      </div>
    </section>
